...
const INSTRUMENTS = [
  "Piano", "Guitar", "Synth", "Drums", "Bass", "Strings", "Brass",
];
...
import {
  ...
  Heart, ShoppingCart, ChevronDown, Clock, X, Check, Zap,
...
interface LicenseOption {
  type: LicenseType;
  label: string;
  price: number | null; // null = non renseigné par le producteur
...
const GENRES = [
  "Tous", "Trap", "Rnb", "Pop", "Hip-Hop", "Afrobeat",
  "Drill", "Reggaeton", "Lo-Fi", "Soul", "Dancehall", "Electro", "Jazz",
];
const MOODS = [
  "Tous", "Dark", "Chill", "Uplifting", "Energetic",
  "Romantic", "Aggressive", "Melancholic",
];
const BPM_RANGES = [
...
/**
 * Lit directement basicPrice / premiumPrice / exclusivePrice du beat.
 * Si un champ est null/undefined → price = null → carte désactivée.
 */
function buildLicenses(beat: any): LicenseOption[] {
...
export default function CataloguePage() {
  ...
  const activeBeat = useMemo(() => allBeats.find((b) => b.id === activeBeatId), [allBeats, activeBeatId]);
  ...
  const togglePlay = (id: string) => {
    if (activeBeatId === id) {
      ...
      if (audioRef.current) {
        ...
        audioRef.current.play().then(() => setIsBuffering(false)).catch(() => setIsBuffering(false));
        setActiveBeatId(id);
  ...
  const resetFilters = () => {
    setSelectedGenre("Tous"); setSelectedMood("Tous");
    setSelectedBpm(BPM_RANGES[0]); setSelectedInstrument("Tous"); setSearchQuery("");
  };
  ...
  const { beats, total } = useMemo(() => {
    ...
    let result = allBeats.filter((beat) => {
      if (q) {
        const ok = beat.title?.toLowerCase().includes(q)
          || beat.description?.toLowerCase().includes(q)
          || beat.seller?.displayName?.toLowerCase().includes(q)
          || beat.seller?.username?.toLowerCase().includes(q);
        if (!ok) return false;
      }
      if (selectedGenre !== "Tous" && !beat.genre?.includes(selectedGenre)) return false;
      if (selectedMood !== "Tous" && !beat.mood?.includes(selectedMood)) return false;
      if (selectedBpm.min !== undefined && (beat.bpm ?? 0) < selectedBpm.min) return false;
      if (selectedBpm.max !== undefined && (beat.bpm ?? 0) > selectedBpm.max) return false;
      if (selectedInstrument !== "Tous" && !(Array.isArray(beat.instruments) && beat.instruments.includes(selectedInstrument))) return false;
      return true;
    });
  ...
  }, [allBeats, searchQuery, selectedGenre, selectedMood, selectedBpm, selectedInstrument, sortBy]);
  ...
  return (
    <div className="relative min-h-screen bg-gradient-premium">
          ...
          <div className="mx-auto max-w-7xl">
            <div className="glass rounded-2xl p-4 flex flex-col lg:flex-row gap-4 items-center">
              ...
              >
                ...
                {activeFilterCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-brand-gold text-slate-900 text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center">{activeFilterCount}</span>
                )}
              </button>

              <div className="hidden lg:flex items-center gap-3">
                <SelectFilter value={selectedGenre} onChange={setSelectedGenre} options={GENRES} active={selectedGenre !== "Tous"} />
              ...
              <div className="hidden lg:flex items-center gap-1 glass rounded-xl p-1">
                <button onClick={() => setViewMode("grid")} className={`p-2 rounded-lg transition-all ${viewMode === "grid" ? "bg-brand-gold/20 text-brand-gold" : "text-slate-400 hover:text-white"}`}>
                  <Grid3X3 className="w-5 h-5" />
                </button>
                <button onClick={() => setViewMode("list")} className={`p-2 rounded-lg transition-all ${viewMode === "list" ? "bg-brand-gold/20 text-brand-gold" : "text-slate-400 hover:text-white"}`}>
            ...
            {(activeFilterCount > 0 || searchQuery) && (
              <div className="flex flex-wrap items-center gap-2 mt-3 px-1">
                <span className="text-[11px] text-slate-600 font-semibold uppercase tracking-wider">Filtres actifs :</span>
                {searchQuery && <ActivePill label={`"${searchQuery}"`} onRemove={() => setSearchQuery("")} color="blue" />}
                {selectedGenre !== "Tous" && <ActivePill label={`${GENRE_EMOJI[selectedGenre] ?? ""} ${selectedGenre}`} onRemove={() => setSelectedGenre("Tous")} color="gold" />}
                {selectedMood !== "Tous" && <ActivePill label={selectedMood} onRemove={() => setSelectedMood("Tous")} color="violet" />}
                {selectedBpm.min !== undefined && <ActivePill label={`⚡ ${selectedBpm.label}`} onRemove={() => setSelectedBpm(BPM_RANGES[0])} color="emerald" />}
                {selectedInstrument !== "Tous" && <ActivePill label={selectedInstrument} onRemove={() => setSelectedInstrument("Tous")} color="blue" />}
                <button onClick={resetFilters} className="text-[11px] text-slate-600 hover:text-red-400 transition-colors ml-1 flex items-center gap-1">
            ...
            {showFilters && (
              <div className="glass rounded-2xl p-6 mt-4 space-y-5 lg:hidden">
                <div>
                  <p className="text-xs text-slate-400 mb-3 font-semibold uppercase tracking-wider">Genre</p>
                  <div className="flex flex-wrap gap-2">
                    {GENRES.map((g) => (
                      <button key={g} onClick={() => setSelectedGenre(g)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${selectedGenre === g ? "bg-brand-gold border-brand-gold text-slate-900" : "bg-white/5 border-white/10 text-slate-400 hover:border-white/25 hover:text-white"}`}>
                        {g !== "Tous" && GENRE_EMOJI[g]} {g}
                        {selectedGenre === g && g !== "Tous" && <Check className="w-3 h-3" />}
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-slate-400 mb-3 font-semibold uppercase tracking-wider">Ambiance</p>
                  <div className="flex flex-wrap gap-2">
                    {MOODS.map((m) => (
                      <button key={m} onClick={() => setSelectedMood(m)} className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${selectedMood === m ? "bg-brand-gold border-brand-gold text-slate-900" : "bg-white/5 border-white/10 text-slate-400 hover:border-white/25 hover:text-white"}`}>
                        {m}{selectedMood === m && m !== "Tous" && <Check className="w-3 h-3 inline ml-1" />}
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-slate-400 mb-3 font-semibold uppercase tracking-wider">Instrument</p>
                  <div className="flex flex-wrap gap-2">
                    {["Tous", ...INSTRUMENTS].map((i) => (
                      <button key={i} onClick={() => setSelectedInstrument(i)} className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${selectedInstrument === i ? "bg-brand-gold border-brand-gold text-slate-900" : "bg-white/5 border-white/10 text-slate-400 hover:border-white/25 hover:text-white"}`}>
                        {i}{selectedInstrument === i && i !== "Tous" && <Check className="w-3 h-3 inline ml-1" />}
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  <p className="text-xs text-slate-400 mb-3 font-semibold uppercase tracking-wider">BPM</p>
                  <div className="flex flex-wrap gap-2">
                    {BPM_RANGES.map((r) => (
                      <button key={r.label} onClick={() => setSelectedBpm(r)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all ${selectedBpm.label === r.label ? "bg-brand-gold border-brand-gold text-slate-900" : "bg-white/5 border-white/10 text-slate-400 hover:border-white/25 hover:text-white"}`}>
          ...
          <div className="mx-auto max-w-7xl">
            <div className="flex items-center justify-between mb-6">
              <p className="text-slate-400 text-sm">
                ...
                {searchQuery && !loading && <span className="text-slate-600 ml-1">pour <span className="text-white/70">"{searchQuery}"</span></span>}
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="glass rounded-3xl p-12 text-center">
                ...
                {(activeFilterCount > 0 || searchQuery) && (
                  <button onClick={resetFilters} className="mt-5 px-5 py-2.5 rounded-xl bg-brand-gold text-slate-900 text-sm font-black hover:brightness-110 transition-all">Réinitialiser</button>
                )}
              </div>
            ) : viewMode === "grid" ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {beats.map((beat) => {
                  ...
                  return (
                    <div key={beat.id} className={`glass group rounded-2xl p-6 text-center hover:scale-[1.03] hover:shadow-xl hover:shadow-black/30 transition-all duration-200 relative ${isActive ? "ring-2 ring-brand-gold/50 bg-brand-gold/5" : ""}`}>
                      <button onClick={() => toggleLike(beat.id)} className={`absolute top-3 right-3 p-1.5 rounded-lg transition-all ${isLiked ? "text-rose-400 opacity-100" : "text-slate-600 opacity-0 group-hover:opacity-100 hover:text-rose-400"}`}>
                        ...
                        {isPlaying && (
                          <div className="absolute inset-0 bg-black/55 flex items-end justify-center gap-[3px] pb-1.5">
                            {[0.1, 0.22, 0.14].map((d, i) => (
                              <span key={i} className="w-[3px] bg-brand-gold rounded-sm" style={{ height: "50%", animation: `eqBar 0.55s ease-in-out ${d}s infinite alternate` }} />
                            ))}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-white/10">
                        <span className="text-brand-gold font-bold text-sm">
                          dès {Number(beat.basicPrice ?? 0).toFixed(2)}€
                        </span>
                        <div className="flex items-center gap-2">
                          <button onClick={() => togglePlay(beat.id)} className="w-8 h-8 rounded-full bg-brand-gold/10 flex items-center justify-center hover:bg-brand-gold/20 transition-colors">
                            {isPlaying ? <Pause className="w-3.5 h-3.5 text-brand-gold fill-current" /> : <Play className="w-3.5 h-3.5 text-brand-gold fill-current ml-0.5" />}
                          </button>
                          <button onClick={() => setLicenseTarget(beat)} className="btn-primary px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-1 flex-shrink-0">
                  ...
                  return (
                    <div key={beat.id} className={`glass rounded-xl p-4 flex items-center gap-4 transition-all hover:bg-white/5 ${isActive ? "ring-1 ring-brand-gold/40 bg-brand-gold/5" : ""}`}>
                      ...
                      <button onClick={() => togglePlay(beat.id)} className="w-12 h-12 rounded-full bg-brand-gold/10 flex items-center justify-center hover:bg-brand-gold/20 transition-colors flex-shrink-0">
                        {isPlaying ? <Pause className="w-5 h-5 text-brand-gold fill-current" /> : <Play className="w-5 h-5 text-brand-gold ml-0.5" />}
                      </button>
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="hidden md:flex items-center gap-6 text-xs text-slate-400 flex-shrink-0">
                        ...
                        {dur && <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{dur}</span>}
                        {genre0 && <span>{genre0}</span>}
                      </div>
                      <button onClick={() => toggleLike(beat.id)} className={`glass p-2 rounded-lg hover:bg-white/10 transition-colors flex-shrink-0 ${isLiked ? "text-rose-400" : ""}`}>
                        <Heart className={`w-4 h-4 ${isLiked ? "fill-current" : ""}`} />
                      </button>
                      <div className="text-brand-gold font-bold text-sm flex-shrink-0">
                        dès {Number(beat.basicPrice ?? 0).toFixed(2)}€
                      </div>
                      <button onClick={() => setLicenseTarget(beat)} className="btn-primary p-2 rounded-lg text-xs font-semibold flex items-center justify-center hover:scale-105 transition-transform">
                        <ShoppingCart className="w-4 h-4" />
                      </button>
                    </div>
                  );
        ...
        <div className="fixed bottom-0 left-0 right-0 glass z-50 border-t border-brand-gold/10 backdrop-blur-xl bg-[#0a0a0f]/95 animate-in slide-in-from-bottom-5">
          <div className="mx-auto max-w-[1400px] px-4 py-3 flex items-center justify-between h-20">
            <div className="flex items-center gap-3 w-1/3 min-w-[200px]">
              ...
              <button onClick={() => toggleLike(activeBeat.id)} className={`ml-2 p-1.5 rounded-lg transition-colors flex-shrink-0 ${likedIds.has(activeBeat.id) ? "text-rose-400" : "text-slate-500 hover:text-white"}`}>
            ...
            <div className="flex flex-col items-center justify-center gap-1.5 w-1/3 max-w-[600px]">
              <div className="flex items-center gap-5">
                <button onClick={() => togglePlay(activeBeat.id)} className="w-8 h-8 rounded-full bg-white hover:scale-105 text-black flex items-center justify-center transition-transform">
                  {isBuffering
              ...
              <div className="flex items-center gap-3 w-full text-[11px] text-slate-400 font-medium">
                ...
                <div className="relative flex-1 h-1 bg-white/10 rounded-full group flex items-center cursor-pointer">
                  ...
                  <div className="h-full bg-white group-hover:bg-brand-gold transition-colors rounded-full" style={{ width: `${progress}%` }}>
            ...
            <div className="w-[30%] min-w-[200px] flex justify-end gap-6 items-center">
              <div className="flex items-center gap-2 group w-32">
                ...
                <div className="relative flex-1 h-1.5 bg-white/10 rounded-full flex items-center">
                  ...
                  <div className="h-full bg-slate-300 group-hover:bg-brand-gold transition-colors rounded-full" style={{ width: `${(isMuted ? 0 : volume) * 100}%` }} />
                </div>
              </div>
              <div className="h-6 w-px bg-white/10 hidden sm:block" />
              <div className="flex items-center gap-3">
                <div className="text-xs font-bold text-brand-gold hidden sm:block">
                  dès {Number(activeBeat.basicPrice ?? 0).toFixed(2)}€
                </div>
                <button onClick={() => setLicenseTarget(activeBeat)} className="btn-primary p-2 rounded-lg text-xs font-semibold flex items-center justify-center hover:scale-105 transition-transform">
                  <ShoppingCart className="w-4 h-4" />
                </button>
              </div>
            </div>
      ...
      {/* ── Modal sélection de licence ── */}
      <LicensePickerModal
        beat={licenseTarget}
        onClose={() => setLicenseTarget(null)}
        onAdd={addToCart}
        isPlaying={activeBeatId === licenseTarget?.id && isPlayingAudio}
...
function SelectFilter({ value, onChange, options, active }: {
  value: string; onChange: (v: string) => void; options: string[]; active: boolean;
}) {
  return (
    <div className="relative">
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className={`appearance-none rounded-xl px-4 py-3 pr-10 text-white text-sm focus:outline-none cursor-pointer transition-all border ${active ? "bg-brand-gold/15 border-brand-gold/50 text-brand-gold" : "bg-white/5 border-white/10 focus:border-brand-gold/50"}`}
      >
        {options.map((o) => <option key={o} value={o} className="bg-[#0d0d12] text-white">{o}</option>)}
      </select>
      <ChevronDown className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none ${active ? "text-brand-gold" : "text-slate-500"}`} />
    </div>
  );
}

function ActivePill({ label, onRemove, color }: {
